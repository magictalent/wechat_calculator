...
class APIs {
  ...
  static Future<void> sendPushNotification(
    ...
    try {
      ...
      const projectID = 'calculator-3d7a0';
  ...
  static Future<bool> sendFriendRequest(String email) async {
    ...
    if (data.docs.isNotEmpty && data.docs.first.id != user.uid) {
      ...
      final targetUser = ChatUser.fromJson(data.docs.first.data());
      final requestId =
          '${user.uid}_${targetUser.id}_${DateTime.now().millisecondsSinceEpoch}';
  ...
  // for getting pending friend requests
  ...
  static Future<void> acceptFriendRequest(FriendRequest request) async {
    ...
    await firestore.collection('friend_requests').doc(request.id).update({
      'status': 'accepted',
    });

    // Add each other to my_users
    await firestore
        .collection('users')
        .doc(user.uid)
        .collection('my_users')
    ...
    if (requester.exists) {
      final requesterUser = ChatUser.fromJson(requester.data()!);
      await sendPushNotification(
        requesterUser,
  ...
  static Future<void> rejectFriendRequest(FriendRequest request) async {
    await firestore.collection('friend_requests').doc(request.id).update({
      'status': 'rejected',
    });
  }

  // for adding an chat user for our conversation (legacy method - now use friend request)
